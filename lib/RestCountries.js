...
class RestCountries {
  ...
  constructor() {
    this.apiHost = 'https://countries.craftypixels.com/v2';
  }
  ...
  findByName(countryName) {
    let options = {
      url: this.apiHost + `/v2/name/${countryName}`
    };
  ...
  findByFullName(countryName) {
    let options = {
      url: this.apiHost + `/v2/name/${countryName}?fullText=true`
    };
  ...
  findByIsoCountryCode(isoCountryCode) {
    let options = {
      url: this.apiHost + `/v2/alpha/${isoCountryCode}`
    };
  ...
  findByIsoCountryCodes(isoCountryCodes) {
    let options = {
      url: this.apiHost + `/v2/alpha?codes=${isoCountryCodes}`
    };
  ...
  findByCurrency(currency) {
    let options = {
      url: this.apiHost + `/v2/currency/${currency}`
    };
  ...
  findByLanguageCode(languageCode) {
    let options = {
      url: this.apiHost + `/v2/lang/${languageCode}`
    };
  ...
  findByCapitalCity(city) {
    let options = {
      url: this.apiHost + `/v2/capital/${city}`
    };
  ...
  findByRegion(region) {
    let options = {
      url: this.apiHost + `/v2/region/${region}`
    };
